...
internal data class AppTabRequests(
    ...
    val iptvScrollToTopRequests: Flow<Unit>,
            ...
            when (selectedTab) {
                AppScreenTab.Home -> {
                    key(state.homeContentGeneration) {
                        HomeScreen(
                            ...
                            onOpenSportsTab = actions.onOpenSportsTab,
                            onPlaySportsChannel = actions.onPlayLiveChannel,
                            onPlaySportsReplay = actions.onPlaySportsReplay,
                            onAddIptvPlaylist = actions.onIptvAddProvider,
                        )
                    }
                }
                ...
                AppScreenTab.Iptv -> {
                    com.nuvio.app.core.contracts.IptvHubContentAccess.current()?.Render(
                        modifier = Modifier.fillMaxSize(),
                        onPosterClick = { meta -> actions.onPosterClick?.invoke(meta) },
                        onPlayLiveChannel = actions.onPlayLiveChannel,
                ...
                AppScreenTab.Sports -> {
                    com.nuvio.app.core.contracts.SportsHubContentAccess.current()?.Render(
                        ...
                        onPlayChannel = actions.onPlayLiveChannel,
                        onAddPlaylist = actions.onIptvAddProvider,
                        // Recordings are registry-registered VOD ids — native detail pipeline.
                        onOpenRecording = { id ->
                            com.nuvio.app.core.contracts.LivePlaybackAccess.current().recordingPreview(id)?.let { actions.onPosterClick?.invoke(it) }
                        },
                        ...
                        onPlayReplay = actions.onPlaySportsReplay,
                    )
                }
