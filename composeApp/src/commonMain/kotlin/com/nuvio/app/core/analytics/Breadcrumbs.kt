 ...
 * `$screen` only ever reports the host Activity/controller name, and `app_exit` is reported one
 * launch later so it has no live session to correlate with. Every breadcrumb therefore goes two
 * ways at once — a live analytics event through [AnalyticsSink] for same-session correlation
 * (`$exception`, `live_playback_freeze`), and a persisted note through [CrashWriter] that the
 * platform's process-exit reporter attaches to the next-launch `app_exit` event.
 *
 * Only code identities travel here — route class names, engine ids, container extensions —
 * never content titles, URLs, or anything else about *what* is being watched.
 */
object Breadcrumbs {
    ...
    /** Registered by hosts that have a process-exit reporter; null hosts just skip persistence. */
    var crashWriter: CrashWriter? = null

    private var capture: (String, Map<String, Any>) -> Unit = AnalyticsSink::capture
    ...
    /**
     * Call when playback has actually produced output (first frame / moving position), not when
    ...
    internal fun resetForTest(capture: (String, Map<String, Any>) -> Unit = AnalyticsSink::capture) {
